...
class SO3Matrix(_base.SOMatrixBase):
    ...
    def exp(cls, phi):
        ...
        small_angle_mask = utils.isclose(angle, 0.0)
        small_angle_inds = small_angle_mask.nonzero(as_tuple=False).squeeze_(dim=1)

        if len(small_angle_inds) > 0:
            mat[small_angle_inds] = torch.eye(cls.dim, dtype=phi.dtype).expand_as(
        ...
        if len(large_angle_inds) > 0:
            ...
            axis = phi[large_angle_inds] / angle.unsqueeze(dim=1).expand(
                len(angle), cls.dim
            )
            s = (
                angle.sin()
                .unsqueeze_(dim=1)
                .unsqueeze_(dim=2)
                .expand_as(mat[large_angle_inds])
            )
            c = (
                angle.cos()
                .unsqueeze_(dim=1)
    ...
    def from_quaternion(cls, quat, ordering="wxyz"):
        ...
        if not utils.allclose(quat.norm(p=2, dim=1), 1.0):
    ...
    def inv_left_jacobian(cls, phi):
        ...
        small_angle_inds = small_angle_mask.nonzero(as_tuple=False).squeeze_(dim=1)
        if len(small_angle_inds) > 0:
            jac[small_angle_inds] = torch.eye(cls.dof, dtype=phi.dtype).expand_as(
        ...
        if len(large_angle_inds) > 0:
            ...
            A = hacha * torch.eye(cls.dof, dtype=phi.dtype).unsqueeze_(dim=0).expand_as(
                jac[large_angle_inds]
            )
            B = (1.0 - hacha) * utils.outer(axis, axis)
            C = -ha * cls.wedge(axis)
    ...
    def left_jacobian(cls, phi):
        ...
        if len(large_angle_inds) > 0:
            ...
            B = (1.0 - s / angle).unsqueeze_(dim=1).unsqueeze_(dim=2).expand_as(
                jac[large_angle_inds]
            ) * utils.outer(axis, axis)
            C = ((1.0 - c) / angle).unsqueeze_(dim=1).unsqueeze_(dim=2).expand_as(
                jac[large_angle_inds]
    ...
    def log(self):
        ...
        cos_angle = (0.5 * utils.trace(mat) - 0.5).clamp_(-1.0, 1.0)
    ...
    def rotx(cls, angle_in_radians):
        ...
        mat = angle_in_radians.new_empty(
            angle_in_radians.shape[0], cls.dim, cls.dim
        ).zero_()
        mat[:, 0, 0] = 1.0
        mat[:, 1, 1] = c
        mat[:, 1, 2] = -s
    ...
    def to_quaternion(self, ordering="wxyz"):
        ...
        near_zero_mask = utils.isclose(qw, 0.0)
    ...
    def to_rpy(self):
        ...
        pitch = torch.atan2(
            -mat[:, 2, 0], torch.sqrt(mat[:, 0, 0] ** 2 + mat[:, 1, 0] ** 2)
        )
        yaw = pitch.new_empty(pitch.shape)
        roll = pitch.new_empty(pitch.shape)

        near_pi_over_two_mask = utils.isclose(pitch, np.pi / 2.0)
        near_pi_over_two_inds = near_pi_over_two_mask.nonzero(as_tuple=False).squeeze_(
            dim=1
        )

        near_neg_pi_over_two_mask = utils.isclose(pitch, -np.pi / 2.0)
        near_neg_pi_over_two_inds = near_neg_pi_over_two_mask.nonzero(
            as_tuple=False
        ).squeeze_(dim=1)

        remainder_inds = (
            (near_pi_over_two_mask | near_neg_pi_over_two_mask)
            .logical_not()
        ...
        if len(near_pi_over_two_inds) > 0:
            yaw[near_pi_over_two_inds] = 0.0
            roll[near_pi_over_two_inds] = torch.atan2(
                mat[near_pi_over_two_inds, 0, 1], mat[near_pi_over_two_inds, 1, 1]
            )
        ...
        if len(remainder_inds) > 0:
            sec_pitch = 1.0 / pitch[remainder_inds].cos()
        ...
        return torch.cat(
            [roll.unsqueeze_(dim=1), pitch.unsqueeze_(dim=1), yaw.unsqueeze_(dim=1)],
            dim=1,
        ).squeeze_()
