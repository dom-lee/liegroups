...
class TestSE3(unittest.TestCase):
    ...
    def test_dot(self):
        T = np.array([[0, 0, -1, 0.1],
                      ...
                      [0, 0, 0,  1]])
        T2 = T.dot(T)
        assert np.allclose((SE3.from_matrix(T).dot(SE3.from_matrix(T))).as_matrix(), T2)
    ...
    def test_curlywedge_curlyvee(self):
        xi = [1, 2, 3, 4, 5, 6]
    ...
    def test_left_jacobian(self):
        xi1 = [1, 2, 3, 4, 5, 6]
        assert np.allclose(
            SE3.left_jacobian(xi1).dot(SE3.inv_left_jacobian(xi1)), np.identity(6)
        )

        xi2 = [0, 0, 0, 0, 0, 0]
        assert np.allclose(
            SE3.left_jacobian(xi2).dot(SE3.inv_left_jacobian(xi2)), np.identity(6)
        )
