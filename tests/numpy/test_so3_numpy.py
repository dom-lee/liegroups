...
class TestSO3(unittest.TestCase):
    ...
    def test_from_rpy_to_rpy(self):
        r, p, y = np.pi / 4.0, np.pi / 3.0, np.pi
    ...
    def test_rotx(self):
        ...
        C_expected = np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]])
        assert np.allclose(C_got.mat, C_expected)

    def test_roty(self):
        C_got = SO3.roty(np.pi / 2)
        C_expected = np.array([[0, 0, 1], [0, 1, 0], [-1, 0, 0]])
        assert np.allclose(C_got.mat, C_expected)

    def test_rotz(self):
        C_got = SO3.rotz(np.pi / 2)
        C_expected = np.array([[0, -1, 0], [1, 0, 0], [0, 0, 1]])
        assert np.allclose(C_got.mat, C_expected)
    ...
    def test_quaternion(self):
        ...
        assert np.allclose(SO3.from_quaternion(q5).mat, SO3.from_quaternion(q6).mat)
    ...
    def test_left_jacobians(self):
        ...
        assert np.allclose(
            left_jacobian_small.dot(inv_left_jacobian_small), np.identity(3)
        )
    ...
    def test_transform_vectorized(self):
        C = SO3.exp(np.pi * np.ones(3) / 4)
        pt1 = np.array([1, 2, 3])
        pt2 = np.array([4, 5, 3])
