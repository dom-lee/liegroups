...
def test_dot():
    ...
    assert utils.allclose(Tpth_SE2, Tpth) and utils.allclose(Tpth_SE2[0:2], Tpt)
...
def test_dot_batch():
    ...
    assert T1pt1_SE2.shape == (T1.shape[0], pt1.shape[0]) and utils.allclose(
        T1pt1_SE2, T1pt1
    )
    ...
    assert (
        T1pt1h_SE2.shape == (T1.shape[0], pt1h.shape[0])
    ...
    assert T1pt2_SE2.shape == (T1.shape[0], pt2.shape[0]) and utils.allclose(
        T1pt2_SE2, T1pt2
    )
    ...
    assert (
        T1pt2h_SE2.shape == (T1.shape[0], pt2h.shape[0])
    ...
    T1pts = torch.bmm(
        ...
        .transpose(2, 1),
    ).transpose(2, 1) + T1[:, 0:2, 2].unsqueeze(dim=1).expand(
        T1.shape[0], pts.shape[0], pts.shape[1]
    )
    T1pts_SE2 = T1_SE2.dot(pts)
    assert (
        T1pts_SE2.shape == (T1.shape[0], pts.shape[0], pts.shape[1])
    ...
    assert (
        T1ptsh_SE2.shape == (T1.shape[0], ptsh.shape[0], ptsh.shape[1])
    ...
    T1ptsbatch = torch.bmm(T1[:, 0:2, 0:2], ptsbatch.transpose(2, 1)).transpose(
        2, 1
    ) + T1[:, 0:2, 2].unsqueeze(dim=1).expand(ptsbatch.shape)
    T1ptsbatch_SE2 = T1_SE2.dot(ptsbatch)
    assert (
        T1ptsbatch_SE2.shape == ptsbatch.shape
    ...
    assert (
        T1ptshbatch_SE2.shape == ptshbatch.shape
    ...
    T2ptsbatch = torch.matmul(T2[0:2, 0:2], ptsbatch.transpose(2, 1)).transpose(
        2, 1
    ) + T1[:, 0:2, 2].unsqueeze(dim=1).expand(ptsbatch.shape)
    T2ptsbatch_SE2 = T2_SE2.dot(ptsbatch)
    assert (
        T2ptsbatch_SE2.shape == ptsbatch.shape
    ...
    assert (
        T2ptshbatch_SE2.shape == ptshbatch.shape
...
def test_odot():
    ...
    odot13 = torch.cat(
        [SE2.odot(p1, directional=True), torch.zeros(3).unsqueeze_(dim=0)], dim=0
    )
    odot2 = SE2.odot(p2)
    odot3 = SE2.odot(p3)
...
def test_perturb_batch():
    ...
    xi = torch.Tensor([0.3, 0.2, 0.1])
    T_copy1.perturb(xi)
    assert utils.allclose(T_copy1.as_matrix(), (SE2.exp(xi).dot(T)).as_matrix())

    xis = torch.Tensor([[0.3, 0.2, 0.1], [-0.1, -0.2, -0.3]])
    T_copy2.perturb(xis)
    assert utils.allclose(T_copy2.as_matrix(), (SE2.exp(xis).dot(T)).as_matrix())
...
def test_normalize_batch():
    ...
    T.rot.mat.add_(0.1)
    assert (SE2.is_valid_matrix(T.as_matrix()) == torch.ByteTensor([0, 0, 0])).all()

    T.normalize(inds=[0, 2])
    assert (SE2.is_valid_matrix(T.as_matrix()) == torch.ByteTensor([1, 0, 1])).all()
...
def test_inv_batch():
    ...
    assert utils.allclose(
        T.dot(T.inv()).as_matrix(), SE2.identity(T.trans.shape[0]).as_matrix()
    )
