...
def test_identity():
    C = SO2.identity()
    assert isinstance(C, SO2) and C.mat.dim() == 2 and C.mat.shape == (2, 2)


def test_identity_batch():
    C = SO2.identity(5)
    assert isinstance(C, SO2) and C.mat.dim() == 3 and C.mat.shape == (5, 2, 2)

    C_copy = SO2.identity(5, copy=True)
    assert (
        isinstance(C_copy, SO2)
...
def test_dot_batch():
    C1 = SO2(torch.Tensor([[0, -1], [1, 0]]).expand(5, 2, 2))
    C2 = SO2(torch.Tensor([[-1, 0], [0, -1]]))
    ...
    assert C1pt1_SO2.shape == (C1.mat.shape[0], pt1.shape[0]) and utils.allclose(
        C1pt1_SO2, C1pt1
    )

    C1pt2 = torch.matmul(C1.mat, pt2)
    C1pt2_SO2 = C1.dot(pt2)
    assert C1pt2_SO2.shape == (C1.mat.shape[0], pt2.shape[0]) and utils.allclose(
        C1pt2_SO2, C1pt2
    )
    ...
    assert (
        C1pts_SO2.shape == (C1.mat.shape[0], pts.shape[0], pts.shape[1])
    ...
    assert (
        C1ptsbatch_SO2.shape == ptsbatch.shape
    ...
    assert (
        C2ptsbatch_SO2.shape == ptsbatch.shape
...
def test_left_jacobians():
    ...
    assert utils.allclose(
        torch.mm(left_jacobian_small, inv_left_jacobian_small), torch.eye(2)
    )

    left_jacobian_big = SO2.left_jacobian(phi_big)
    inv_left_jacobian_big = SO2.inv_left_jacobian(phi_big)
    assert utils.allclose(
        torch.mm(left_jacobian_big, inv_left_jacobian_big), torch.eye(2)
    )


def test_left_jacobians_batch():
    phis = torch.Tensor([0.0, np.pi / 2])
...
def test_exp_log_batch():
    C = SO2.exp(torch.Tensor([-1.0, 0.0, 1.0]))
...
def test_perturb():
    ...
    C.perturb(phi)
    assert utils.allclose(C.as_matrix(), (SO2.exp(phi).dot(C_copy)).as_matrix())
...
def test_perturb_batch():
    ...
    phi = torch.Tensor([0.1])
    C_copy1.perturb(phi)
    assert utils.allclose(C_copy1.as_matrix(), (SO2.exp(phi).dot(C)).as_matrix())

    phis = torch.Tensor([0.1, 0.2, 0.3])
    C_copy2.perturb(phis)
    assert utils.allclose(C_copy2.as_matrix(), (SO2.exp(phis).dot(C)).as_matrix())
...
def test_adjoint():
    C = SO2.exp(torch.Tensor([np.pi / 4]))
    assert (C.adjoint() == torch.Tensor([1.0])).all()
